...
// 环境变量控制trace级别日志输出
// GO_PLAY_LOG_TRACE=1 开启trace输出
const logEnv = "GO_PLAY_LOG_TRACE"

const contextLogKey = "CONTEXT_LOG_KEY"
...
func WithFieldsContext(ctx context.Context, fields ...string) Logger {
	logger, ok := ctx.Value(contextLogKey).(Logger)
	if !ok {
...
func TraceContext(ctx context.Context, args ...interface{}) {
	if traceEnabled {
		switch l := ctx.Value(contextLogKey).(type) {
		case *zapLogWrapper:
			// 保护 l 或者 l.l 不可为空
			if l == nil || l.l == nil {
				DefaultLogger.Trace(args...)
